...
class Model_schedule extends CI_Model {
	...
	function update_mst_barge($data){
		// $this->db->where('barge_id > 0');
		return $this->db->update('mst_barge', $data);
	}

	function update_mst_barge_tanggal($id,$data){
	...
	function get_trans_po($id,$id2) {
		$this->db->select('SUM(trans_po.quantity) as sumqty, trans_po.po_res_number, trans_po.barge_id');
		$this->db->from('trans_po');
		$this->db->join('mst_storage', 'trans_po.storage_id = mst_storage.storage_id');
		$this->db->group_by('trans_po.posting_date');
	...
	function get_trans_forecast_last($id) {
		$this->db->select('trans_forecast.*, barge_name, prioritas, tanggal_bongkar, durasi_bongkar');
		$this->db->from('trans_forecast');
		$this->db->join('mst_storage', 'trans_forecast.storage_id = mst_storage.storage_id');
	...
	function get_trans_forecast_last_barge($id1,$id2) {
		$this->db->select('trans_forecast.*, barge_name, prioritas, tanggal_bongkar, durasi_bongkar');
		$this->db->from('trans_forecast');
		$this->db->join('mst_storage', 'trans_forecast.storage_id = mst_storage.storage_id');
		$this->db->join('mst_barge', 'trans_forecast.barge_id = mst_barge.barge_id');
		$this->db->where('mst_storage.storage_id=', $id1);
		$this->db->where('mst_barge.barge_id=', $id2);
		$this->db->order_by('trans_forecast.trans_date', 'DESC');
		$this->db->limit(1);
		$query = $this->db->get();
	...
	function get_trans_smartfill_out($id,$tgl) {
		...
		$this->db->where('trans_smartfill.date=', $tgl);
